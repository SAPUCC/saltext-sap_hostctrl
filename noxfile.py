...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=tests/*",
            "--include=src/saltext/sap_hostctrl/*",
        )
        ...
        session.run(
            ...
            "--omit=src/saltext/sap_hostctrl/*",
            "--include=tests/*",
        )
        try:
            session.run(
                "coverage", "report", "--show-missing", "--include=src/saltext/sap_hostctrl/*"
            )
            ...
            #    "--include=src/saltext/sap_hostctrl/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap_hostctrl/config/schemas",
    )
